...
def get_clu_centers(args, toTrModels, dataloaders, centers, label='init'):# label='init' | 'clf'
    ...
    if label == 'init':  # Calculate based on init pseudo labels
        zt_clu_cents = torch.zeros((n_clu, z_dim)).type(FloatTensor)
        at_clu_cents = torch.zeros((n_clu, a_dim)).type(FloatTensor)
        n_clu_samples = torch.zeros((n_clu)).type(FloatTensor)
        ...
        zt_clu_cents = zt_clu_cents / n_clu_samples.unsqueeze(1).repeat(1, z_dim)
        at_clu_cents = at_clu_cents / n_clu_samples.unsqueeze(1).repeat(1, a_dim)
