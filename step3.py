...
from pseudo_label_analysis import PseudoLabelTracker, collect_pseudo_from_loader
...
def track_pseudo_labels_epoch(args, epoch, dataloaders, toTrModels, tracker):
    """Evaluate pseudo-label (clu) and model predictions vs. GT on full target set."""
    for key in toTrModels:
        toTrModels[key].eval()
    y_true_all, clu_all = [], []
    clf_all, clfsu_all = [], []
    te_iter = enumerate(dataloaders['te_loader_tgt'])
    n_batches = len(dataloaders['te_loader_tgt'])
    for itern in range(n_batches):
        (feats_tgt, lbls_tgt, atts_tgt, clu_tgt) = te_iter.__next__()[1]
        ...
        y_true_all.append(lbls_tgt.numpy())
        clu_all.append(clu_tgt.numpy())
        clf_all.append(t_y_pred.cpu().numpy())
        clfsu_all.append(t_su_pred.cpu().numpy())
    y_true = np.concatenate(y_true_all)
    clu = np.concatenate(clu_all)
    clf_pred = np.concatenate(clf_all)
    clfsu_pred = np.concatenate(clfsu_all)
    rec = tracker.evaluate_batch(
        y_true, clu, clf_pred=clf_pred, clf_su_pred=clfsu_pred, epoch=epoch, phase="target_test"
    )
    print(
        "  [Pseudo analysis] ep={:d} clu_ACC={:.4f} boundary={:.4f} "
        "clf_ACC={:.4f} ClfSU_boundary={:.4f}".format(
            epoch,
            rec["clu_cluster_acc"],
            rec["clu_boundary_acc"],
            rec.get("clf_cluster_acc", 0.0),
            rec.get("clfsu_boundary_acc", 0.0),
        )
    )
    for key in toTrModels:
...
def train_step3(args, dataloaders, toTrModels, nonTrModels, lossFunctions, optimizers, centers, pseudo_tracker=None):
    shr_nc = args.shr_nc
    unk_clf_id = shr_nc  # open-set "unknown" logit index in Clf (shr_nc+1 classes)
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch+1) < 2:  # (epoch+1) % 10 == 0
            ...
            centers['zt_clu_cents'], centers['at_clu_cents'] = get_clu_centers(
                args, toTrModels, dataloaders, centers, label='init')
        else:
            centers['zt_clu_cents'], centers['at_clu_cents'] = get_clu_centers(
                args, toTrModels, dataloaders, centers, label='clf')
        ...
        for itern in range(larger_batches):
            ...
            t_conf_shr_inds = clu_tgt_var < shr_nc
            t_conf_unk_inds = clu_tgt_var >= shr_nc

            yt_conf_shr = clu_tgt_var[t_conf_shr_inds]
            yt_conf_unk = clu_tgt_var[t_conf_unk_inds] * 0 + unk_clf_id
        ...
        if pseudo_tracker is not None:
            track_pseudo_labels_epoch(args, epoch, dataloaders, toTrModels, pseudo_tracker)
    ...
    if pseudo_tracker is not None:
        out_sub = os.path.join(
            args.pseudo_analysis_dir, args.src + '2' + args.tgt
        )
        os.makedirs(out_sub, exist_ok=True)
        pseudo_tracker.output_dir = out_sub
        csv_path = pseudo_tracker.save(prefix='step3_epoch_tracking')
        pseudo_tracker.plot_tracking(prefix='step3_epoch_tracking')
        print('  [Pseudo analysis] saved tracking to', csv_path)
